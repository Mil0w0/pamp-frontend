...
type IconType = 'check' | 'alert' | 'clock' | 'x' | 'info' | 'calendar' | 'users' | 'file'
...
export function StatusIndicator({
    ...
    const getIcon = () => {
        ...
        if (icon) {
            switch (icon) {
                ...
                case 'x': return <XCircle className={iconClass} />
        ...
        switch (type) {
            ...
            case 'late': return <AlertCircle className={iconClass} />
        }
    }
    ...
    const getTextColor = () => {
        switch (type) {
            ...
            case 'pending': return 'text-gray-700 dark:text-gray-300'
            case 'late': return 'text-red-700 dark:text-red-300'
        }
    }
    ...
    return (
        <div className={cn('flex items-center gap-2', className)}>
            <Badge variant={getBadgeVariant()} className={cn('flex items-center gap-1', sizeClass)}>
                {getIcon()}
                {text}
            </Badge>
            {description && (
